document.addEventListener('DOMContentLoaded', () => {
    ...
    const renderTable = () => {
        ...
        filteredProducts.forEach(p => {
            ...
            // 0 (Blanco) -> 2 (Rojo, Captura Referencia)
            // 2 (Rojo) <-> 1 (Verde) [Toggle sin borrar diferencia]
            ...
            checkArea.onclick = (e) => {
                ...
                if (currentData.checkState === 0 || !currentData.checkState) {
                    newState = 2; // Entra a ROJO y captura cantidad actual
                    redQty = currentData.qty || 0;
                } else if (currentData.checkState === 2) {
                    newState = 1; // Cambia a VERDE (Mantiene diff)
                } else {
                    newState = 2; // Cambia a ROJO (Mantiene diff, no sobreescribe redQty)
                }
            ...
            if (editBtn) {
                editBtn.onclick = (e) => {
                    ...
                    if (choice === "1") {
                        ...
                        inventoryData[p] = { ...inventoryData[p], checkState: 0, redQty: null };
                        localStorage.setItem('inventoryData', JSON.stringify(inventoryData));
                        renderTable();
                    }
                };
    ...
    if (downloadBtn) {
        downloadBtn.onclick = async () => {
            ...
            row2.values = ["PASILLO Nº 2 🛒 DE FERIA DEL ESTE 🛒"];
